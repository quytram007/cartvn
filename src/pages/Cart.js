...
const Cart = () =>{
    ...
    const handleOnReduceQuantity = (product,index)=>{
        const _product = {...product,quantity:product.quantity-1,productTotal:product.productPrice*(product.quantity-1)}
        let _cart = [...cart]
        ...
        if (product.isChecked===true){
            ...
            let _paymentProduct = [...payment.product]
                ...
                setPayment({...payment,product:_paymentProduct,quantity:payment.quantity-1,total:payment.total-product.productPrice});
        }
    }
    ...
    const handleOnChoiceProduct = (check,value)=>{
        if(check){
            ...
            setPayment({...payment,product:product,
    ...
    const handleOnChoiceVoucher = (voucher) =>{
        setVoucerChoice(voucher)
    }
    const handleOnApply = ()=>{
        setIsShowVoucher(false)
        setPayment({...payment,total:payment.total-(payment.total*voucherChoice.discount/100),afterDiscount:payment.total})
    }
    const handShowVoucher=()=>{
        setIsShowVoucher(true)
        if (voucherChoice.discount !== 0)
        setPayment({...payment,total:payment.afterDiscount})
    }
    ...
    <div className='cart grid'>
        {cart.map((value,index)=>(
            ...
            >
                ...
                <div className='product-total'>{value.productTotal}</div>
            </div>
        ))}
        <div className='payment grid'>
                ...
                <div className='voucher'>
                    ...
                    <div className='choice-voucher' onClick={()=>handShowVoucher()}>Chọn Hoặc Nhập Mã</div>
                </div>  
            ...
            <div className='voucher'>
                <div className='header-voucher'>
                    <div className='input-code'>
                        ...
                        <input className='voucher-code' value={voucherCode} onChange={(e)=>setVoucherCode(e.target.value)}></input>
                        <button className='voucher-button' onClick={()=>handleOnApplyVoucher()}>Áp Dụng</button>
                    </div>
                </div>
                        ...
                        <div className='voucher-freeship'>
                            {
                                listVoucher.map((voucher,index)=>(
                                    ...
                                    <div className='child-voucher' key={index} onClick={()=>handleOnChoiceVoucher(voucher)}>
                                        <div className='img'>
                                            <i className="fal fa-shipping-fast"></i>
                                            <div className='title'>Miễn Phí Vận Chuyển</div>
                                        </div>
                                        <div className='decripstion'>
                                            Bạn sẽ được giảm {(voucher.discount/1000).toFixed(3)} phí vận chuyển
                                        </div>
                                        <input
                                        type={'radio'}
                ...
                <div className='footer-voucher'>
                    <button className='back' onClick={()=>setIsShowVoucher(false)}>Trở Lại</button>
                    <button className='ok' onClick={()=>handleOnApply()}>OK</button>
                </div>
            </div>
        </div>
        }
